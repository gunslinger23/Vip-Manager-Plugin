...
#define PLUGIN_AUTHOR "Totenfluch,gunslinger"
...
{
	//Create natives
	CreateNative("Vip_GrantVip", NativeGrantVip);
	CreateNative("Vip_DeleteVip", NativeDeleteVip);
...
public void OnPluginStart() {
	...
	Format(createTableQuery, sizeof(createTableQuery), 
		"CREATE TABLE IF NOT EXISTS `vip` ( \
 		`Id` bigint(20) NOT NULL AUTO_INCREMENT, \
  		...
  		`playername` varchar(36) COLLATE utf8mb4_unicode_ci NOT NULL, \
  		`playerid` varchar(20) COLLATE utf8mb4_unicode_ci NOT NULL, \
  		`enddate` timestamp NOT NULL DEFAULT '0000-00-00 00:00:00', \
  		`admin_playername` varchar(36) COLLATE utf8mb4_unicode_ci NOT NULL, \
  		`admin_playerid` varchar(20) COLLATE utf8mb4_unicode_ci NOT NULL, \
 		 PRIMARY KEY (`Id`), \
	...
	AutoExecConfig_SetFile("Vip");
	AutoExecConfig_SetCreateFile(true);
	
	g_hFlag = AutoExecConfig_CreateConVar("vip_flag", "19", "20=Custom6, 19=Custom5 etc. Numeric Flag See: 'https://wiki.alliedmods.net/Checking_Admin_Flags_(SourceMod_Scripting)' for Definitions ---- Multiple flags seperated with Space: '16 17 18 19' !!");
	g_hTestVipDuration = AutoExecConfig_CreateConVar("vip_testVipDuration", "15", "Test Vip duration in minutes");
	
	AutoExecConfig_CleanFile();
	AutoExecConfig_ExecuteFile();
	
	RegAdminCmd("sm_vipadmin", cmdtVIP, ADMFLAG_ROOT, "Opens the VIP menu");
...
public Action openVipPanel(int client, int args) {
	if (g_bIsVip[client]) {
		...
		Format(getDatesQuery, sizeof(getDatesQuery), "SELECT timestamp,enddate,DATEDIFF(enddate, NOW()) as timeleft FROM vip WHERE playerid = '%s';", playerid);
...
public Action cmdListVips(int client, int args) {
	...
	Format(showOffVIPQuery, sizeof(showOffVIPQuery), "SELECT playername,playerid FROM vip WHERE NOW() < enddate;");
...
public void grantVip(int admin, int client, int duration, int reason) {
	...
	Format(addVipQuery, sizeof(addVipQuery), "INSERT IGNORE INTO `vip` (`Id`, `timestamp`, `playername`, `playerid`, `enddate`, `admin_playername`, `admin_playerid`) VALUES (NULL, CURRENT_TIMESTAMP, '%s', '%s', CURRENT_TIMESTAMP, '%s', '%s');", clean_playername, playerid, clean_admin_playername, admin_playerid);
	SQL_TQuery(g_DB, SQLErrorCheckCallback, addVipQuery);
	
	char updateTime[1024];
	if (reason != 3)
		Format(updateTime, sizeof(updateTime), "UPDATE vip SET enddate = DATE_ADD(enddate, INTERVAL %i MONTH) WHERE playerid = '%s';", duration, playerid);
	else
		Format(updateTime, sizeof(updateTime), "UPDATE vip SET enddate = DATE_ADD(enddate, INTERVAL %i MINUTE) WHERE playerid = '%s';", duration, playerid);
...
public void grantVipEx(int admin, char playerid[20], int duration, char[] pname) {
	...
	char addVipQuery[4096];
	Format(addVipQuery, sizeof(addVipQuery), "INSERT IGNORE INTO `vip` (`Id`, `timestamp`, `playername`, `playerid`, `enddate`, `admin_playername`, `admin_playerid`) VALUES (NULL, CURRENT_TIMESTAMP, '%s', '%s', CURRENT_TIMESTAMP, '%s', '%s');", pname, playerid, clean_admin_playername, admin_playerid);
	SQL_TQuery(g_DB, SQLErrorCheckCallback, addVipQuery);
...
public void OnClientPostAdminCheck(int client) {
	...
	char cleanUp[256];
	Format(cleanUp, sizeof(cleanUp), "DELETE FROM vip WHERE enddate < NOW();");
...
public void loadVip(int client) {
	...
	Format(isVipQuery, sizeof(isVipQuery), "SELECT * FROM vip WHERE playerid = '%s' AND enddate > NOW();", playerid);
...
public void showAllVIPsToAdmin(int client) {
	...
	Format(selectAllVIPs, sizeof(selectAllVIPs), "SELECT playername,playerid FROM vip WHERE NOW() < enddate;");
...
public void deleteVip(char[] playerid) {
	...
	Format(deleteVipQuery, sizeof(deleteVipQuery), "DELETE FROM vip WHERE playerid = '%s';", playerid);
...
public void extendVip(int client, int userTarget, int duration) {
	...
	Format(updateQuery, sizeof(updateQuery), "UPDATE vip SET enddate = DATE_ADD(enddate, INTERVAL %i MONTH) WHERE playerid = '%s';", duration, playerid);
	SQL_TQuery(g_DB, SQLErrorCheckCallback, updateQuery);
	
	Format(updateQuery, sizeof(updateQuery), "UPDATE vip SET playername = '%s' WHERE playerid = '%s';", clean_playername, playerid);
	SQL_TQuery(g_DB, SQLErrorCheckCallback, updateQuery);
...
public void listUsers(int client) {
	...
	Format(listVipsQuery, sizeof(listVipsQuery), "SELECT playername,playerid FROM vip WHERE enddate > NOW();");
...
public int listVipsMenuHandler(Handle menu, MenuAction action, int client, int item) {
	if (action == MenuAction_Select) {
		...
		Format(detailsQuery, sizeof(detailsQuery), "SELECT playername,playerid,enddate,timestamp,admin_playername,admin_playerid FROM vip WHERE playerid = '%s';", cValue);
